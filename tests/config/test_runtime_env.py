...
def test_load_runtime_env_merges_dotenv_and_env(tmp_path: Path) -> None:
    ...
    env_file.write_text("HARMONY_LOG_LEVEL=DEBUG\nCACHE_ENABLED=false\n", encoding="utf-8")

    runtime_env = load_runtime_env(env_file=env_file, base_env={"CACHE_ENABLED": "true"})
...
def test_env_variables_override_dotenv_and_defaults(tmp_path: Path) -> None:
    ...
    assert config.database.url == "postgresql+psycopg://override:harmony@localhost:5432/app"
