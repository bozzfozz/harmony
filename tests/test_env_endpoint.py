...
def test_env_endpoint_exposes_configuration() -> None:
    ...
    assert features["enable_legacy_routes"] == config_snapshot.features.enable_legacy_routes
    assert (
    ...
    assert workers["enabled_override"] == config_snapshot.environment.workers.enabled_override
    assert workers["enabled_raw"] == config_snapshot.environment.workers.enabled_raw
    assert (
        workers["visibility_timeout_s"] == config_snapshot.environment.workers.visibility_timeout_s
    )
    assert (
        workers["watchlist_interval_s"] == config_snapshot.environment.workers.watchlist_interval_s
    )
    assert (
    ...
    assert orchestrator["visibility_timeout_s"] == settings.orchestrator.visibility_timeout_s
    assert orchestrator["poll_interval_ms"] == settings.orchestrator.poll_interval_ms
    assert orchestrator["poll_interval_max_ms"] == settings.orchestrator.poll_interval_max_ms
