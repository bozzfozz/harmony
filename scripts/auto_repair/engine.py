...
PINNED_NODE_VERSION = "20.17.1"
PINNED_NPM_VERSION = "10.8.2"
...
class NodeToolchainFixer(Fixer):
    issue_id = "node.toolchain"
    description = "Align Node.js and npm versions with repository pins"

    def matches(self, context: RepairContext) -> bool:
        if "node" not in context.command.name and "npm" not in context.command.name:
            return False
        output = context.combined_output.lower()
        return 'engine "node"' in output or "node" in output and "incompatible" in output

    def apply(self, context: RepairContext, logger: ReasonTraceLogger) -> FixOutcome:
        logger.emit(
            step="DECIDE",
            why="Detected Node.js toolchain drift",
        ...
        ensure_file(REPO_ROOT / ".nvmrc", PINNED_NODE_VERSION)
        ensure_file(REPO_ROOT / ".node-version", PINNED_NODE_VERSION)
        npm_version_file = REPO_ROOT / "frontend" / ".npm-version"
        ensure_file(npm_version_file, PINNED_NPM_VERSION)

        logger.emit(
            step="FIX",
            why="Rewriting Node.js version pins",
            command=", ".join(updated_files) if updated_files else "No pin updates required",
            result="OK",
            nxt="VERIFY",
        )
        ...
        message = (
            "Node version pins already aligned"
            if not updated_files
            else "Node version pins updated"
        )
        warnings: list[str] = []
        try:
            ...
            if node_path:
                completed = subprocess.run(
                    [node_path, "--version"], check=False, capture_output=True, text=True
                )
                logger.emit(
                    step="VERIFY",
                    why="Checking active Node.js runtime",
                    command=f"{node_path} --version",
                    result=completed.stdout.strip() or completed.stderr.strip(),
                    nxt="RE-RUN",
                )
                if completed.returncode == 0 and PINNED_NODE_VERSION not in completed.stdout:
                    warnings.append("Installed Node.js runtime differs from repository pin")
            else:
                warnings.append("Node.js executable not found on PATH")
        except OSError as exc:
            warnings.append(f"Failed to verify Node.js runtime: {exc}")
...
class NodeIntegrityFixer(Fixer):
    issue_id = "node.eintegrity"
    description = "Repair npm EINTEGRITY / tarball corruption"

    def matches(self, context: RepairContext) -> bool:
        output = context.combined_output.lower()
        return "eintegrity" in output or "corrupt" in output or "tarball" in output
    ...
    def apply(self, context: RepairContext, logger: ReasonTraceLogger) -> FixOutcome:
        ...
        def ensure_registry(path: Path) -> None:
            desired = f"registry={DEFAULT_REGISTRY}\n"
            lines = []
            if path.exists():
                lines = path.read_text(encoding="utf-8").splitlines()
            filtered = [line for line in lines if not line.startswith("registry=")]
            filtered.insert(0, desired.strip())
            path.write_text("\n".join(filtered) + "\n", encoding="utf-8")

        logger.emit(
            step="DECIDE",
            why="npm EINTEGRITY detected",
            command=context.command.name,
            nxt="FIX",
        )
        ensure_registry(REPO_ROOT / ".npmrc")
        ensure_registry(frontend_dir / ".npmrc")
        ...
        for cmd in commands:
            ...
            try:
                ...
                logger.emit(
                    step="FIX",
                    why="Running npm recovery command",
                    command=f"(cd frontend && {display})",
                    result=f"exit={completed.returncode}",
                    nxt="VERIFY" if cmd is commands[-1] else "FIX",
                )
                if completed.returncode != 0:
                    return FixOutcome(
                        ...
                        description=self.description,
                        success=False,
                        message=f"Command '{display}' exited with {completed.returncode}",
        ...
        return FixOutcome(
            ...
            description=self.description,
            success=True,
            message="npm cache flushed and lockfile regenerated",
            commands=executed,
        )


class NodeRegistryFixer(Fixer):
    issue_id = "node.registry"
    description = "Handle npm registry 403/availability issues"

    def matches(self, context: RepairContext) -> bool:
        output = context.combined_output.lower()
        return "e403" in output or "403" in output and "registry" in output

    def apply(self, context: RepairContext, logger: ReasonTraceLogger) -> FixOutcome:
        logger.emit(
            step="DECIDE",
            why="Registry 403 detected",
            command=context.command.name,
            nxt="FIX",
        )
        warnings = [
            "npm registry returned 403. Existing node_modules will be reused if available.",
            "Investigate proxy/firewall settings and retry once connectivity is restored.",
        ]
        logger.emit(
            step="FIX",
            why="Switching to WARN-only mode for registry outage",
            command="skip npm reinstall",
            result="WARN",
            nxt="DONE",
        )
        return FixOutcome(
            ...
            description=self.description,
            success=False,
            message="Registry unavailable; command skipped",
            warnings=warnings,
            resolved_without_rerun=True,
        )


class NodeEnvFixer(Fixer):
    issue_id = "node.env"
    description = "Ensure NODE_ENV allows devDependencies during install"

    def matches(self, context: RepairContext) -> bool:
        if "npm" not in context.command.name:
            return False
        node_env = context.env.get("NODE_ENV") or os.environ.get("NODE_ENV", "")
        return node_env.lower() == "production"

    def apply(self, context: RepairContext, logger: ReasonTraceLogger) -> FixOutcome:
        logger.emit(
            step="DECIDE",
            why="NODE_ENV=production blocks devDependencies",
            command=context.command.name,
            nxt="FIX",
        )
        logger.emit(
            step="FIX",
            why="Setting NODE_ENV=development for install",
        ...
        return FixOutcome(
            ...
            description=self.description,
            success=True,
            message="NODE_ENV adjusted to development for npm install",
            env_updates={"NODE_ENV": "development"},
        )
