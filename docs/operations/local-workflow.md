...
| `make supply-guard`       | `scripts/dev/supply_guard.sh`       | Stellt sicher, dass keine Paketmanager-Artefakte vorhanden sind und die statischen Frontend-Dateien existieren. |
| `make vendor-frontend`    | `scripts/dev/vendor_frontend.sh`    | Informiert darüber, dass keine Vendoring-Schritte mehr erforderlich sind. |
| `make vendor-frontend-reset` | `scripts/dev/vendor_frontend.sh --reset` | Alias für den Hinweis, dass kein Vendoring notwendig ist. |
...
4. **Optionaler Offline-Test:** Das neue statische Frontend benötigt kein Vendoring mehr; Tests können direkt mit den Assets unter `frontend-static/` durchgeführt werden.
...
### `make supply-guard`
- **Fehlende Assets:** Erstelle die statischen Dateien unter `frontend-static/` erneut oder synchronisiere den Branch.
...
- Die Kommandos `make vendor-frontend` und `make vendor-frontend-reset` dienen nur noch der Abwärtskompatibilität und geben einen Hinweis aus, dass keine weiteren Schritte notwendig sind.
